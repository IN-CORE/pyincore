...
cat requirements.tmp | grep -v ' *#.*' | grep -v '^$' | grep -v "^${TESTPYPI}$" > requirements.ver

# create a markdown file with what we found.
cat << EOF > versions.md
# ${MODULE} dependencies version list

This file is autogenerated and lists the latest versions of all dependencies as found on $(date). The table will list package (pip and conda), and latest version found.
...
for p in $(cat requirements.imports ); do
  ...
  LATEST=$(grep "^${PACKAGE}==" 2>/dev/null requirements.ver | sed "s/^${PACKAGE}==//")
  CONDA=$(echo $r | sed -e 's/fastjsonschema/python-fastjsonschema/' \
