    ...
    def compute_bridge_damage_analysis(self):
        ...
        if len(fragility_set) > 0:
            for item in self.bridges.inventory_set:
                ...
                id = item['id']
                hazard_val = self.hazardsvc.get_earthquake_hazard_value(self.hazard_dataset_id,fragility_set[id]['demandType'].lower(), fragility_set[id]['demandUnits'], center_point.x, center_point.y)
                print(hazard_val)
