...
class SeasideCGEModel(BaseAnalysis):
    ...
    def run(self):
        ...
        HHTABLE = pd.read_csv(
            self.get_input_dataset("HHTABLE").get_file_path("csv"), index_col=0
        )
        ...
        PRIVRET.loc[H] = SAM.loc[Z, H].sum(0) - (
            SAM.loc[H, F].sum(1) + SAM.loc[H, GX].sum(1)
        )
        ...
        def set_equation(filename):
            ...
            line = ExprM(vars, m=CX0.loc[I]) * (
                (PD.loc(I)) / ExprM(vars, m=PW0.loc[I])
            ...
            line2 = (
                ((RA.loc(L) / ExprM(vars, m=RA0.loc[L])).sum(L) / 5)
        ...
        def run_solver(cons_filename, temp_file_name):
            ...
            solver_io = "nl"
            stream_solver = (
                self.get_parameter("print_solver_output")
                if self.get_parameter("print_solver_output") is not None
                else False
        ...
        self.set_result_csv_data(
            "Seaside_Sims", sims_result, name="Seaside_Sims", source="dataframe"
        )

        # Prepare cge output
        total_employment_original = FD0.loc[L, I].sum(0).sum(0)
        total_employment_change = (
            vars.get("FD", x=soln[-1]).loc[L, I].sum(0).sum(0)
            - total_employment_original
        )
        total_employment_percentage = (
            total_employment_change / total_employment_original
        )

        domestic_supply_original = DS0.sum(0)
        domestic_supply_change = (
            vars.get("DS", x=soln[-1]).sum(0) - domestic_supply_original
        )
        domestic_supply_percentage = domestic_supply_change / domestic_supply_original

        DY = vars.get("Y", x=soln[-1]) - Y0
        DY.loc[H] = pd.Series(
            vars.get("Y", x=soln[-1]).loc[H] / vars.get("CPI", x=soln[-1]).loc[H]
            - Y0.loc[H]
        )
        ...
        cge_output = pd.DataFrame(
            {
                ...
                "Amount of Change": [
                    "{:.2f}".format(total_employment_change),
                    "{:.2f}".format(domestic_supply_change),
                    "",
                    "{:.2f}".format(HH1_change),
                    "{:.2f}".format(HH2_change),
                    "{:.2f}".format(HH3_change),
                    "{:.2f}".format(HH4_change),
                ...
                "Percent Change": [
                    "{:.2f}%".format(total_employment_percentage * 100),
        ...
        self.set_result_csv_data(
            "Seaside_output", cge_output, name="Seaside_output", source="dataframe"
        )
