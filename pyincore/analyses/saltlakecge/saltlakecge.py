...
class SaltLakeCGEModel(BaseAnalysis):
    ...
    def run(self):
        ...
        sector_shocks = pd.read_csv(
            self.get_input_dataset("sector_shocks").get_file_path("csv"), index_col=0
        )

        self.salt_lake_city_cge(
            iNum, SAM, BB, JOBCR, MISCH, EMPLOY, OUTCR, sector_shocks
        )

    def salt_lake_city_cge(
        self, iNum, SAM, BB, JOBCR, MISCH, EMPLOY, OUTCR, sector_shocks
    ):
        """
        ...
        CN0.loc[I] = (
            (B.loc[I, IG].mul(N0.loc[K, IG].sum(0), axis="columns"))
            .sum(1)
            .div(P0.loc[I], axis="index")
            .div(1.0 + TAUN.loc[GS, I].sum(0), axis="index")
            .transpose()
        )
        ...
        TESTA1.loc[F, I] = (
            R0.loc[F, I].mul(RA0.loc[F], axis="index")
            * (1 + TAUFX_SUM.loc[F, I])
        ...
        GAMMA.loc[I] = DS0.loc[I] / (
            ((ALPHA.loc[F, I] * FD0.loc[F, I] ** (RHO.loc[I])).sum(0))
            ** (1 / RHO.loc[I])
        )
        ...
        CMI0.loc[L] = EMPLOY.loc[IG, L].sum(0) - (
            JOBCOR.loc[H, L].mul(HW0.loc[H], axis="index")
        ).sum(0)
        ...
        def set_equation(filename):
            ...
            line = ExprM(vars, m=GAMMA.loc[I]) * (
                (
                    ExprM(vars, m=ALPHA.loc[F, I])
                    * (FD.loc(F, I) ** ExprM(vars, m=RHO.loc[I]))
                ).sum(F)
            ...
            print("FDEQ(F,I)")
            left = (
                R.loc(F, I) * RA.loc(F) * ExprM(vars, m=1 + TAUFX_SUM.loc[F, I])
            ) * (FD.loc(F, I) ** ExprM(vars, m=1 - RHO.loc[I]))
            right = ~(
                PVA.loc(I)
                * (
                    ExprM(vars, m=ALPHA.loc[F, I] * (GAMMA.loc[I] ** RHO.loc[I]))
            ...
            line = ExprM(vars, m=CX0.loc[I]) * (
                ...
                / ExprM(vars, m=PW0.loc[I] * (1 + TAUQ.loc[GS, I].sum(0)))
            ...
            ) ** ExprM(vars, m=ETAIX.loc[K, I] * 0.1)
            ...
            line2 = (
                ((RA.loc(L) / ExprM(vars, m=RA0.loc[L])).sum(L) / 8)
            ...
            line4 = (
                (
                    (ExprM(vars, m=PIT0.loc[GI, H]) * ExprM(vars, m=HH0.loc[H])).sum(GI)
                    + (ExprM(vars, m=TAUH0.loc[G, H]) * ExprM(vars, m=HH0.loc[H])).sum(
                        G
                    )
                )
                / (
            ...
            line = ExprM(vars, m=CMO0.loc[CM1]) * (
                (ExprM(vars, m=EXWGEO.loc[CM1].sum(0)) / RA.loc(FW1))
            ...
            line = ExprM(vars, m=CMO0.loc[CM2]) * (
                (ExprM(vars, m=EXWGEO.loc[CM2].sum(0)) / RA.loc(FW2))
            ...
            line = ExprM(vars, m=CMO0.loc[CM3]) * (
                (ExprM(vars, m=EXWGEO.loc[CM3].sum(0)) / RA.loc(FW3))
            ...
            line = ExprM(vars, m=CMO0.loc[CM4]) * (
                (ExprM(vars, m=EXWGEO.loc[CM4].sum(0)) / RA.loc(FW4))
                ** (ExprM(vars, m=ECOMO.loc[CM4]))
            )
            ...
            line = ExprM(vars, m=CMI0.loc[L]) * (
                (RA.loc(L) / (CPI.loc(H).sum(H) / 8)) ** (ExprM(vars, m=ECOMI.loc[L]))
            )
    ...
    def get_spec(self):
        return {
            "name": "Salt-Lake-cge",
            "description": "Computable general equilibrium (CGE) model for Salt Lake City.",
            "input_parameters": [
                {
                    "id": "model_iterations",
                    "required": True,
            ...
            "output_datasets": [
                {
                    "id": "domestic-supply",
                    "parent_type": "",
                    "description": "A CSV file of resulting domestic supply.",
                    "type": "incore:Employment",
                },
                {
                    "id": "gross-income",
                    "parent_type": "",
                    "description": "A CSV file of resulting gross income.",
                    "type": "incore:Employment",
                },
                {
                    "id": "pre-disaster-factor-demand",
                    "parent_type": "",
                    "description": "A CSV file of factor demand before disaster.",
                    "type": "incore:FactorDemand",
                },
                {
                    "id": "post-disaster-factor-demand",
                    "parent_type": "",
                    "description": "A CSV file of resulting factor-demand.",
                    "type": "incore:FactorDemand",
                },
                {
                    "id": "household-count",
                    "parent_type": "",
                    "description": "A CSV file of household count.",
