...
class CombinedWindWaveSurgeBuildingLoss(BaseAnalysis):
    ...
    def run(self):
        ...
        structure_cost = self.get_input_dataset("structural_cost").get_dataframe_from_csv()

        combined_loss = self.get_combined_loss(wind_damage, surge_wave_damage, flood_damage, buildings, content_cost,
                                               structure_cost)

        # Create the result dataset
        self.set_result_csv_data("result", combined_loss, self.get_parameter("result_name") + "_loss",
                                 "dataframe")

        return True
    
    def get_combined_loss(self, wind_dmg: pd.DataFrame, sw_dmg: pd.DataFrame, flood_dmg: pd.DataFrame, buildings:
                          pd.DataFrame, content_cost: pd.DataFrame, structure_cost: pd.DataFrame):
        ...
        new_combined_df = pd.merge(combined_df, buildings[["guid", "arch_flood"]], on="guid")

        # Create a result data frame for the loss calculations
        loss_df = new_combined_df[['guid']].copy()

        # Compute content and structural loss
        loss_df['cont_loss'] = new_combined_df.apply(lambda row: np.dot(row[['f_DS_0', 'f_DS_1', 'f_DS_2']],
                                                                        content_cost.iloc[int(row['arch_flood']-1),
                                                                        1:4]), axis=1)

        loss_df['roof_loss'] = new_combined_df.apply(lambda row: np.dot(row[['w_DS_0', 'w_DS_1', 'w_DS_2', 'w_DS_3']],
                                                                        np.array([0.15, 0.5, 0.75, 1])) *
                                                     structure_cost.loc[int(row['arch_flood']-1), "Roofing"], axis=1)

        loss_df['ff_loss'] = new_combined_df.apply(lambda row: np.dot(row[['sw_DS_0', 'sw_DS_1', 'sw_DS_2',
                                                                           'sw_DS_3']], np.array([0.1, 0.5, 0.75, 1])) *
                                                   structure_cost.loc[int(row['arch_flood'] - 1),
                                                   "Flooring and Foundation"], axis=1)
        ...
        def compute_frame_loss(row):
            if row['w_DS_3'] >= row['sw_DS_3']:
                return np.dot(row[['w_DS_0', 'w_DS_1', 'w_DS_2', 'w_DS_3']],np.array([0.25, 0.5, 0.75, 1])) * \
                    structure_cost.loc[int(row['arch_flood'] - 1), "Wood Framing"]
            elif row['sw_DS_3'] > row['w_DS_3']:
                return np.dot(row[['sw_DS_0', 'sw_DS_1', 'sw_DS_2', 'sw_DS_3']], np.array([0.25, 0.5, 0.75, 1])) * \
                    structure_cost.loc[int(row['arch_flood'] - 1), "Wood Framing"]
            else:
                ...
                if pd.isnull(row['sw_DS_3']) and pd.isnull(row['w_DS_3']):
                    ...
                    return np.dot(row[['w_DS_0', 'w_DS_1', 'w_DS_2', 'w_DS_3']], np.array([0.25, 0.5, 0.75,
                                                                                           1])) * structure_cost.loc[
                        int(row['arch_flood'] - 1), "Wood Framing"]

        loss_df['frame_loss'] = new_combined_df.apply(lambda row: compute_frame_loss(row), axis=1)
        ...
        loss_df['total_loss'] = loss_df.apply(lambda row: row['cont_loss'] + row['frame_loss'] + row[
            'roof_loss'] + row['ff_loss'], axis=1)
