...
class HousingRecovery(BaseAnalysis):
            ...
            "input_parameters": [
                {
                    ...
                    "description": "Base year is used to calculate improvement age. It needs to be set to the tax "
                                   "assessment year representing pre-disaster building values. For example for GCAD data which represents improvement valuation before Hurricane Ike impacts.Deafult 2008",
                    "type": int
                },
                {
                   "id":"result_name",
    ...
    def run(self):
        ...
        hse_rec_phm = self.assemble_phm_coefs(hru, dmg_years, hse_recov_value_loss)
        print(hse_rec_phm.shape)
        print(hse_rec_phm)

        # Seasonal/Vacation housing market (SVHM)
        hse_rec_svhm = self.assemble_svhm_coefs(hru, dmg_years, hse_recov_value_loss)
        print(hse_rec_svhm.shape)
        print(hse_rec_svhm)

        hse_rec_phm_index = hse_rec_phm / hse_rec_phm[:, 0:1]
        hse_rec_svhm_index = hse_rec_svhm / hse_rec_svhm[:, 0:1]

        # for year in dmg_years:
            # Primary Housing Market (PHM)
        ...
        neg_count = len(list(filter(lambda x: (x < 0), dmg_years)))
        print(neg_count)

        # Reversing year order using slicing technique
        dmg_years_rev = dmg_years
        ...
        p_bv_yr = ["p_bv_yr{0}".format(i) for i in range(len(dmg_years))]
        index_yr = ["index_yr{0}".format(i) for i in range(len(dmg_years))]
        ...
        hse_recov_value_loss = pd.concat([hse_recov_value_loss, hse_rec_phm1, hse_rec_phm2], axis=1)

        print(hse_recov_value_loss)
        print("finished")

        # a better solution would be to figure out how to not rename columns when reshaping,
        # or find a different reshaping method
        ...
        # display
        # print(hse_recov_value_loss_reshaped[["guid", "strctid", "d_vacationct", "year", "p_bv_yr", "index_yr"]].head(15))
        print(hse_recov_value_loss_reshaped[["strctid", "d_vacationct", "year", "p_bv_yr", "index_yr"]].head(15))
        ...
        print(hse_recov_value_loss_collapse.head(15))
        exit(1)
...
.
    ...
    def assemble_phm_coefs(self, hru, dmg_years, hse_rec):
    ...
    def assemble_svhm_coefs(self, hru, dmg_years, hse_rec):
    ...
    .
        ...
        valloss = np.fromiter(hru.B_SVHM_dmg_year.values(), dtype=float) + hse_rec["value_loss"].to_numpy()[:, np.newaxis]
        d_owner = np.fromiter(hru.B_SVHM_own_year.values(), dtype=float) + hse_rec["d_ownerocc"].to_numpy()[:, np.newaxis]
        mhhinck = np.fromiter(hru.B_SVHM_inc_year.values(), dtype=float) + hse_rec["mhhinck"].to_numpy()[:, np.newaxis]
