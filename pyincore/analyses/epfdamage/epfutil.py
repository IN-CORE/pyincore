...
class EpfUtil:
    ...
    @staticmethod
    def get_mean_damage(dmg_intervals, start_idx, cur_epf, dmg_ratio_tbl):
        ...
        Args:
            ...
            start_idx (int): An initial index of damage intervals, starting at 1 ignores the
                no damage interval.
            cur_epf (obj): A JSON mapping of a geometric object from the inventory: current epf.
        ...
        """
        if "spans" in cur_epf["properties"] and cur_epf["properties"]["spans"] is not None and \
                cur_epf["properties"]["spans"].isdigit():
            n = int(cur_epf["properties"]["spans"])
        elif "SPANS" in cur_epf["properties"] and cur_epf["properties"]["SPANS"] is not None and \
                cur_epf["properties"]["SPANS"].isdigit():
            n = int(cur_epf["properties"]["SPANS"])
        else:
            n = 1
        ...
        if n > 10:
            n = 10
            print("A epf was found with greater than 10 spans: " + str(cur_epf))
    ...
    @staticmethod
    def get_probability_of_exceedence(epf, fragility_set, hazard_val, std_dev, use_liquefaction):
        """Calculates probabilities of exceedance.

        Args:
            epf (obj): A JSON mapping of a geometric object from the inventory: current epf.
            fragility_set (obj): A JSON description of fragility applicable to the epf.
        ...
        for fragility in fragility_set["fragilityCurves"]:
            ...
            if use_liquefaction and 'liq' in epf['properties']:
                fragility = EpfUtil.adjust_fragility_for_liquefaction(fragility, epf['properties']['liq'])
                median = float(fragility['median'])
    ...
    def adjust_fragility_for_liquefaction(fragility_curve, liquefaction):
        ...
        Args:
            fragility_curve (obj): A JSON like description of current fragility curve.
    ...
    def get_retrofit_cost(target_fragility_key):
        """Calculates retrofit cost estimate of a epf.
        ...
        Note:
            ...
            private FeatureDataset epfRetrofitCostEstimate
        ...
        """
        retrofit_cost = 0.0
        if target_fragility_key.lower() == EpfUtil.DEFAULT_FRAGILITY_KEY.lower():
            return retrofit_cost
        else:
            retrofit_code = EpfUtil.get_retrofit_code()
        return retrofit_cost

    @staticmethod
    def get_retrofit_type(target_fragility_key):
        """Get retrofit type by looking up EPF_FRAGILITY_KEYS dictionary.
        ...
        """
        return EpfUtil.EPF_FRAGILITY_KEYS[target_fragility_key.lower()][0] \
            if target_fragility_key.lower() in EpfUtil.EPF_FRAGILITY_KEYS else "none"

    @staticmethod
    def get_retrofit_code(target_fragility_key):
        """Get retrofit code by looking up EPF_FRAGILITY_KEYS dictionary.
        ...
        """
        return EpfUtil.EPF_FRAGILITY_KEYS[target_fragility_key.lower()][1] \
            if target_fragility_key.lower() in EpfUtil.EPF_FRAGILITY_KEYS else "none"
