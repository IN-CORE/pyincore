...
class GasFacilityDamage(BaseAnalysis):
    ...
    def run(self):
        ...
        inventory_set = self.get_input_dataset(
            "gas_facilities").get_inventory_reader()
        ...
        (ds_results, metadata_results) = self.gasfacility_damage_concurrent_futures(
            self.gasfacility_damage_analysis_bulk_input, num_workers,
        ...
        self.set_result_json_data("metadata", metadata_results, name=result_name + "_additional_info")
    ...
    def gasfacility_damage_analysis_bulk_input(self, facilities, hazard, hazard_type, hazard_dataset_id):
                        ...
                        if isinstance(fragility_set_liq.fragility_curves[0], DFR3Curve):
                            ...
                            pgd_limit_states = \
                                fragility_set_liq.calculate_limit_state(
                                    hval_dict_liq, std_dev=hazard_std_dev, inventory_type="gas_facility",
                                    **facility_liq_args)
                        else:
