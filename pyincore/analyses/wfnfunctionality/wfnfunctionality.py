...
class WfnFunctionality(BaseAnalysis):
    ...
    def run(self):
        ...
        # Get network dataset
        network_dataset = NetworkDataset.from_dataset(self.get_input_dataset('wfn_network'))
        ...
        wf_sample_df = pd.DataFrame(
            np.array([np.array(wf_dmg_fs.failure.values[i].split(',')).astype('int')
                      for i in np.arange(wf_dmg_fs.shape[0])]),
            index=wf_dmg_fs.guid.values)

        # Get pipeline damage states
        pp_dmg_fs = self.get_input_dataset('pp_sample_failure_state').get_dataframe_from_csv()
        pp_sample_df = pd.DataFrame(
            np.array([np.array(pp_dmg_fs.failure.values[i].split(',')).astype('int')
        ...
        wf_sample_df1 = nodes_wfn_gdf.loc[:, ['guid', 'nodenwid']].set_index('guid').join(wf_sample_df)
