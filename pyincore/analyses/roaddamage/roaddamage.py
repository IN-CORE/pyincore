...
class RoadDamage(BaseAnalysis):
    ...
    def run(self):
        ...
        user_defined_cpu = 1
        if (
            self.get_parameter("num_cpu") is not None
        ...
        (ds_results, damage_results) = self.road_damage_concurrent_future(
            self.road_damage_analysis_bulk_input,
            num_workers,
            inventory_args,
            repeat(hazard),
            repeat(hazard_type),
    ...
    def road_damage_analysis_bulk_input(
        self,
        roads,
        hazard,
        hazard_type,
        ...
        if hazard_type == "earthquake":
            ...
            if pgd_flag and use_liquefaction and geology_dataset_id is not None:
                liquefaction_resp = self.hazardsvc.post_liquefaction_values(
                    hazard_dataset_id, geology_dataset_id, values_payload
                )
            ...
            if isinstance(selected_fragility_set.fragility_curves[0], DFR3Curve):
                ...
                if not AnalysisUtil.do_hazard_values_have_errors(
                    ...
                    if liquefaction_resp is not None and len(liquefaction_resp) > 0:
                        ...
                        dmg_probability = selected_fragility_set.calculate_limit_state(
                            liq_hval_dict, inventory_type="road", **road_args
                        )

                    dmg_interval = selected_fragility_set.calculate_damage_interval(
                        dmg_probability, hazard_type=hazard_type, inventory_type="road"
                    )
            else:
                raise ValueError(
            ...
            "input_parameters": [
                {
                    ...
                    "studio-default": "road-damage-result",
                    "type": str,
                },
                {
                    "id": "hazard_type",
