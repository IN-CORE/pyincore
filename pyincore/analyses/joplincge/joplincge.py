...
class JoplinCGEModel(BaseAnalysis):
    ...
    def get_spec(self):
        return {
            ...
            "description": "A computable general equilibrium (CGE) model is based on fundamental economic principles."
            "A CGE model uses multiple data sources to reflect the interactions of households,"
            "firms and relevant government entities as they contribute to economic activity."
            "The model is based on (1) utility-maximizing households that supply labor and capital,"
            "using the proceeds to pay for goods and services (both locally produced and imported)"
    ...
    def run(self):
        ...
        # I+G SECTORS
        IG = ["GOODS", "TRADE", "OTHER", "HS1", "HS2", "HS3", "FED", "STATE", "CITY"]

        # INDUSTRY SECTORS
        I = ["GOODS", "TRADE", "OTHER", "HS1", "HS2", "HS3"]
        ...
        # SAM
        SAM = pd.read_csv(
            self.get_input_dataset("SAM").get_file_path("csv"), index_col=0
        )
        # CAPITAL COMP
        BB = pd.read_csv(self.get_input_dataset("BB").get_file_path("csv"), index_col=0)
        BB.rename(
            columns=lambda x: x.strip(), inplace=True
        )  # Skips whitespace around column name from head
        BB.rename(
            index=lambda x: x.strip(), inplace=True
        )  # Skips whitespace around index name from head

        # MISC TABLES
        IOUT = pd.read_csv(
            self.get_input_dataset("IOUT").get_file_path("csv"), index_col=0
        )
        MISC = pd.read_csv(
            self.get_input_dataset("MISC").get_file_path("csv"), index_col=0
        )
        MISCH = pd.read_csv(
            self.get_input_dataset("MISCH").get_file_path("csv"), index_col=0
        )
        LANDCAP = pd.read_csv(
            self.get_input_dataset("LANDCAP").get_file_path("csv"), index_col=0
        )
        EMPLOY = pd.read_csv(
            self.get_input_dataset("EMPLOY").get_file_path("csv"), index_col=0
        )
        IGTD = pd.read_csv(
            self.get_input_dataset("IGTD").get_file_path("csv"), index_col=0
        )
        TAUFF = pd.read_csv(
            self.get_input_dataset("TAUFF").get_file_path("csv"), index_col=0
        )
        JOBCR = pd.read_csv(
            self.get_input_dataset("JOBCR").get_file_path("csv"), index_col=0
        )
        OUTCR = pd.read_csv(
            self.get_input_dataset("OUTCR").get_file_path("csv"), index_col=0
        )
        ...
        # build tax arrays
        TAUFarray = [
            [[0 for k in range(len(Z))] for j in range(len(F))] for i in range(len(G))
        ]
        for i in range(len(GF)):
            for j in range(len(F)):
                for k in range(len(I)):
                    if SAM.loc[F[j], I[k]] != 0 and TAUFF.loc[GF[i], F[j]] != 0:
                        TAUFarray[G.index(GF[i])][j][Z.index(I[k])] = (
                            SAM.loc[GF[i], I[k]] / SAM.loc[F[j], I[k]]
                        )
            ...
            for j in range(len(F)):
                for k in range(len(G)):
                    if SAM.loc[F[j], G[k]] != 0 and TAUFF.loc[GF[i], F[j]] != 0:
                        TAUFarray[G.index(GF[i])][j][Z.index(G[k])] = (
                            SAM.loc[GF[i], G[k]] / SAM.loc[F[j], G[k]]
                        )
        ...
        TAXS1.loc[GNL] = SAM.loc[GNL, ["CYGF"]].sum(1) / SAM.loc[GNL, ["CYGF"]].sum(
            1
        ).sum(0)
        ...
        CMI0.loc[["L1"]] = (
            FD0.loc[["L1"], IG].sum(1)
            - JOBCOR.loc[H, ["L1"]].mul(HW0.loc[H], axis="index").sum(0)
            + CMO0.loc[CM1].sum(0)
        )
        CMI0.loc[["L2"]] = (
            FD0.loc[["L2"], IG].sum(1)
            - JOBCOR.loc[H, ["L2"]].mul(HW0.loc[H], axis="index").sum(0)
            + CMO0.loc[CM2].sum(0)
        )
        CMI0.loc[["L3"]] = (
            FD0.loc[["L3"], IG].sum(1)
            - JOBCOR.loc[H, ["L3"]].mul(HW0.loc[H], axis="index").sum(0)
            + CMO0.loc[CM3].sum(0)
        )

        CMOWAGE.loc[CM] = (
            SAM.loc[CM, ["ROW"]].sum(1).div(CMO0.loc[CM], axis="index").fillna(0.0)
        )

        CMIWAGE.loc[L] = (-1) * (
            SAM.loc[L, ["ROW"]].sum(1).div(CMI0.loc[L], axis="index").fillna(0.0)
        )
        ...
        CX0.loc[I] = (
            SAM.loc[I, ["ROW"]]
            .div(P0.loc[I], axis="index")
            .div(1.0 + TAUQ.loc[GS, I].sum(0), axis="index")
            .sum(1)
        )

        M01.loc[I] = SAM.loc[["ROW"], I].sum(0) / PWM0[I].T

        M0.loc[IP] = SAM.loc[IP, Z].sum(1) - (
            B1.loc[I, IP].sum(0) + SAM.loc[F, IP].sum(0) + SAM.loc[G, IP].sum(0)
        )
        ...
        CN0.loc[I] = (
            B.loc[I, IG]
            .mul(N0.loc[K, IG].sum(0), axis="columns")
            .sum(1)
            .div(P0.loc[I], axis="index")
            .div(1.0 + TAUQ.loc[GS, I].sum(0), axis="index")
            .transpose()
        ...
        DD0.loc[I] = (
            CH0.loc[I, H].sum(1) + CG0.loc[I, G].sum(1) + CN0.loc[I] + V0.loc[I]
        )
        ...
        PVA0.loc[I] = PD0.loc[I] - (
            AD.loc[I, I]
            .mul(P0.loc[I], axis="index")
            .mul(1.0 + TAUQ.loc[GS, I].sum(0).T, axis="index")
            .sum(0)
            .T
        )
        ...
        PRIVRET.loc[H] = (
            SAM.loc[Z, H].sum(0)
            - (SAM.loc[H, F].sum(1) + SAM.loc[H, CM].sum(1) + SAM.loc[H, GX].sum(1))
        ) / HH0.loc[H]
        ...
        A.loc[H, L] = (
            SAM.loc[H, L].div(HW0.loc[H], axis="index")
            / (Y0.loc[L] + SAM.loc[Z, L].sum(0) - SAM.loc[L, IG].sum(1))
        ...
        A.loc[H, LA] = (
            SAM.loc[H, LA].div(HW0.loc[H], axis="index")
            / (Y0.loc[LA] + SAM.loc[Z, LA].sum(0) - SAM.loc[LA, IG].sum(1))
            * (1.0 - TAUFLA.loc[G, LA].sum(0))
        )
        ...
        NKI0 = (
            (M0.loc[I] * PWM0.loc[I]).sum(0)
            - (CX0.loc[I] * PD0.loc[I]).sum(0)
        ...
        Y0.loc[H] = (
            (
                A.loc[H, L]
                .mul(HW0[H], axis="index")
                .div(A.loc[H, L].mul(HW0[H], axis="index").sum(0), axis="columns")
                .mul(Y0.loc[L] - (CMIWAGE.loc[L] * CMI0.loc[L]), axis="columns")
                .mul(1.0 - TAUFL.loc[G, L].sum(0), axis="columns")
            ).sum(1)
            + (A.loc[H, CM].mul((CMOWAGE.loc[CM] * CMO0.loc[CM]), axis="columns")).sum(
                1
            )
            + (
                A.loc[H, LA]
                .mul(HW0[H], axis="index")
                .div(A.loc[H, LA].mul(HW0[H], axis="index").sum(0), axis="columns")
                * (Y0.loc[LA] * (1.0 - TAUFLA.loc[G, LA].sum(0)) + LNFOR0.loc[LA])
            ).sum(1)
            + (
                A.loc[H, K].mul(HW0[H], axis="index")
                / A.loc[H, K].mul(HW0[H], axis="index").sum(0)
                * (Y0[K] * (1.0 - TAUFK.loc[G, K].sum(0)) + KPFOR0.loc[K])
            ).sum(1)
        )
        ...
        GCP0 = (
            CH0.loc[I, H].sum(1).sum(0)
            + CN0.loc[I].sum(0)
            + CG0.loc[I, GN].sum(1).sum(0)
            + CX0.loc[I].sum(0)
            - M0.loc[I].sum(0)
        )
        ...
        def set_equation(filename):
            ...
            # print('CPIEQ(H)')
            line1 = (
                P.loc(I) * ExprM(vars, m=1 + TAUC.loc[GS, I].sum(0)) * CH.loc(I, H)
            ).sum(I)
            line2 = (
                ExprM(vars, m=P0.loc[I] * (1 + TAUQ.loc[GS, I].sum(0))) * CH.loc(I, H)
            ).sum(I)
            ...
            # print('YEQ(H)')
            line1 = (
                ExprM(vars, m=A.loc[H, L])
                * HW.loc(H)
                / (ExprM(vars, m=A.loc[H1, L]) * HW.loc(H1)).sum(H1)
                * (Y.loc(L) - ExprM(vars, m=CMIWAGE.loc[L]) * CMI.loc(L))
                * ExprM(vars, m=1 - TAUFL.loc[G, L].sum(0))
            ).sum(L)
            line2 = (ExprM(vars, m=A.loc[H, CM] * CMOWAGE.loc[CM]) * CMO.loc(CM)).sum(
                CM
            )
            line3 = (
                ExprM(vars, m=A.loc[H, LA])
                * HW.loc(H)
                / (ExprM(vars, m=A.loc[H1, LA]) * HW.loc(H1)).sum(H1)
                * (Y.loc(LA) + LNFOR.loc(LA))
                * ExprM(vars, m=1 - TAUFLA.loc[G, LA].sum(0))
            ).sum(LA)
            line4 = (
                ExprM(vars, m=A.loc[H, K])
                * HW.loc(H)
                / (ExprM(vars, m=A.loc[H1, K]) * HW.loc(H1)).sum(H1)
                * (Y.loc(K) + KPFOR.loc(K))
                * ExprM(vars, m=1 - TAUFK.loc[G, K].sum(0))
            ).sum(K)
            ...
            line1 = ExprM(vars, m=CH0.loc[I, H]) * (
                (YD.loc(H) / ExprM(vars, m=YD0.loc[H]))
                / (CPI.loc(H) / ExprM(vars, m=CPI0.loc[H]))
            ) ** ExprM(vars, m=BETA.loc[I, H])
            line2 = (
                (
                    (P.loc(J) * ExprM(vars, m=1 + TAUC.loc[GS, J].sum(0)))
                    / (
                        ExprM(vars, m=P0.loc[J])
                        * ExprM(vars, m=1 + TAUQ.loc[GS, J].sum(0))
                    )
                )
                ** ExprM(vars, m=LAMBDA.loc[J, I])
            ).prod(0)
            ...
            line = YD.loc(H) - ~(
                (
                    P.loc(I) * CH.loc(I, H) * ExprM(vars, m=1 + TAUC.loc[GS, I].sum(0))
                ).sum(I)
            )
            ...
            line = ExprM(vars, m=DELTA.loc[I]) * (
                FD.loc(F, I) ** ExprM(vars, m=ALPHA.loc[F, I])
            ).prod(F)
            ...
            left = (
                R.loc(F, I)
                * RA.loc(F)
                * ExprM(vars, m=1 + TAUFX_SUM.loc[F, I])
                * FD.loc(F, I)
            )
            ...
            # print('YFEQLA(LA)')
            line = (R.loc(["LAND"], IG) * RA.loc(["LAND"]) * FD.loc(["LAND"], IG)).sum(
                IG
            )
            ...
            line = ExprM(vars, m=CX0.loc[I]) * (
                (PD.loc(I)) / ExprM(vars, m=PWM0.loc[I])
            ...
            # print('DEQ(I)$PWM0(I)')
            line = ExprM(vars, m=D0.loc[I]) * (
                PD.loc(I) / ExprM(vars, m=PWM0.loc[I])
            ) ** ExprM(vars, m=ETAD.loc[I])
            ...
            # print('NEQ(K,I)')
            line = ExprM(vars, m=N0.loc[K, I]) * (
                R.loc(K, I) / ExprM(vars, m=R0.loc[K, I])
            ...
            line2 = (
                ((RA.loc(L) / ExprM(vars, m=RA0.loc[L])).sum(L) / 3)
                / (CPI.loc(H) / ExprM(vars, m=CPI0[H]))
                * (
                    LSEQ1line2pre.sum(0)
                    / (
            ...
            line3 = (
                (ExprM(vars, m=TP.loc[H, G]) / CPI.loc(H)).sum(G)
                / (ExprM(vars, m=TP.loc[H, G]) / ExprM(vars, m=CPI0.loc[H])).sum(G)
            ) ** ExprM(vars, m=ETAPT.loc[H])
            line4 = (
                (
                ...
                / (
                    (ExprM(vars, m=PIT.loc[GI, H]) * HH.loc(H)).sum(GI)
                    + (ExprM(vars, m=TAUH.loc[G, H]) * HH.loc(H)).sum(G)
                )
            ) ** ExprM(vars, m=ETAPIT.loc[H])
            ...
            line = ExprM(vars, m=CMO0.loc[CM1]) * (
                ExprM(vars, m=EXWGEO.loc[CM1]) / RA.loc(["L1"])
            ...
            line = ExprM(vars, m=CMO0.loc[CM2]) * (
                ExprM(vars, m=EXWGEO.loc[CM2]) / RA.loc(["L2"])
            ...
            line = ExprM(vars, m=CMO0.loc[CM3]) * (
                ExprM(vars, m=EXWGEO.loc[CM3]) / RA.loc(["L3"])
            ...
            # print('LASEQ1(LA,I)')
            line = ExprM(vars, m=LAS0.loc[LA, I]) * (
                R.loc(LA, I) / ExprM(vars, m=R0.loc[LA, I])
            ) ** ExprM(vars, m=ETAL.loc[LA, I])
            ...
            line2 = ExprM(vars, m=MI0.loc[H]) * (
                (YD.loc(H) / HH.loc(H))
                / ExprM(vars, m=YD0.loc[H] / HH0.loc[H])
                / (CPI.loc(H) / ExprM(vars, m=CPI0.loc[H]))
            ) ** ExprM(vars, m=ETAYDI.loc[H])
            line3 = (
                (HN.loc(H) / HH.loc(H)) / ExprM(vars, m=HN0.loc[H] / HH0.loc[H])
            ) ** ExprM(vars, m=ETAUI.loc[H])
            line4 = ExprM(vars, m=MO0.loc[H]) * (
                ExprM(vars, m=YD0.loc[H] / HH0.loc[H])
                / (YD.loc(H) / HH.loc(H))
                / (ExprM(vars, m=CPI0.loc[H]) / CPI.loc(H))
            ) ** ExprM(vars, m=ETAYDO.loc[H])
            line5 = (
                ExprM(vars, m=HN0.loc[H] / HH0.loc[H]) / (HN.loc(H) / HH.loc(H))
            ) ** ExprM(vars, m=ETAUO.loc[H])
            ...
            YGEQline5pre = ExprM(vars, m=TAUG.loc[GX, I]).sum(
                I
            )  # first just set it to correct size
            for label in GX:
                YGEQline5pre.m[GX.index(label)][0] = Expr(
                    (ExprM(vars, m=TAUG.loc[[label], I]) * CG.loc(I, GN) * P.loc(I))
                    .sum()
                    .m[0][0]
                )
            line5 = YGEQline5pre
            ...
            for label in GX:
                line6.m[GX.index(label)][0] = Expr(
                    (
                        ExprM(vars, m=TAUFXgx[label].loc[F, I])
                        * RA.loc(F)
                        * R.loc(F, I)
                        * FD.loc(F, I)
                    )
            ...
            for label in GX:
                line7.m[GX.index(label)][0] = Expr(
                    (
                        ExprM(vars, m=TAUFXgx[label].loc[F, GN])
                        * RA.loc(F)
                        * R.loc(F, GN)
                        * FD.loc(F, GN)
                    )
            ...
            right = ExprM(vars, m=AG.loc[I, GN]) * (
                Y.loc(GN) + ExprM(vars, m=GFOR.loc[GN]) * Y.loc(GN)
            )
            ...
            left = (
                ...
                * R.loc(F, GN)
                * RA.loc(F)
                * (1 + ExprM(vars, m=TAUFX_SUM.loc[F, GN]))
            )
            right = ExprM(vars, m=AG.loc[F, GN]) * (
                Y.loc(GN) + ExprM(vars, m=GFOR.loc[GN]) * Y.loc(GN)
            )
            ...
            line2 = (
                CG.loc(I, GN) * P.loc(I) * (1 + ExprM(vars, m=TAUG.loc[GS, I]).sum(GS))
            ).sum(I)
            line3 = (
                FD.loc(F, GN)
                * R.loc(F, GN)
            ...
            line = ExprM(vars, m=TAXS.loc[G, GX]) * (
                Y.loc(GX)
                + GVFOR.loc(GX)
                - ~(ExprM(vars, m=TP.loc[H, GX]) * HH.loc(H)).sum(H)
            )
            ...
            line = (
                Y.loc(H).sum(H)
                + (ExprM(vars, m=TP.loc[H, G]) * HH.loc(H)).sum()
            ...
            # print(SPIEQ)
            # print('LMEQ1')
            left = (ExprM(vars, m=JOBCOR.loc[H, "L1"]) * HW.loc(H)).sum(H) + CMI.loc(
                ["L1"]
            )
            right = (
                FD.loc(["L1"], Z).sum(Z)
                + (HW.loc(H) * ExprM(vars, m=OUTCOR.loc[H, CM1])).sum()
            )
            ...
            # print('LMEQ2')
            left = (ExprM(vars, m=JOBCOR.loc[H, "L2"]) * HW.loc(H)).sum(H) + CMI.loc(
                ["L2"]
            )
            right = (
                FD.loc(["L2"], Z).sum(Z)
                + (HW.loc(H) * ExprM(vars, m=OUTCOR.loc[H, CM2])).sum()
            )
            ...
            # print('LMEQ3')
            left = (ExprM(vars, m=JOBCOR.loc[H, "L3"]) * HW.loc(H)).sum(H) + CMI.loc(
                ["L3"]
            )
            right = (
                FD.loc(["L3"], Z).sum(Z)
                + (HW.loc(H) * ExprM(vars, m=OUTCOR.loc[H, CM3])).sum()
            )
        ...
        def run_solver(cons_filename, temp_file_name):
            ...
            executable_path = (
                self.get_parameter("solver_path")
                if self.get_parameter("solver_path") is not None
                else pyglobals.IPOPT_PATH
            )
        ...
        for ittr in range(iNum):
            ...
            if ittr == 0:  # if it is the first simulation, apply the shock
                ...
                KS0.loc[K, ["HS1"]] = KS0.loc[K, ["HS1"]] * float(
                    (sector_shocks.loc[sector_shocks["sector"] == "HS1"])["shock"]
                )
                KS0.loc[K, ["HS2"]] = KS0.loc[K, ["HS2"]] * float(
                    (sector_shocks.loc[sector_shocks["sector"] == "HS2"])["shock"]
                )
                KS0.loc[K, ["HS3"]] = KS0.loc[K, ["HS3"]] * float(
                    (sector_shocks.loc[sector_shocks["sector"] == "HS3"])["shock"]
                )
                KS0.loc[K, ["GOODS"]] = KS0.loc[K, ["GOODS"]] * float(
                    (sector_shocks.loc[sector_shocks["sector"] == "GOODS"])["shock"]
                )
                KS0.loc[K, ["TRADE"]] = KS0.loc[K, ["TRADE"]] * float(
                    (sector_shocks.loc[sector_shocks["sector"] == "TRADE"])["shock"]
                )
                KS0.loc[K, ["OTHER"]] = KS0.loc[K, ["OTHER"]] * float(
                    (sector_shocks.loc[sector_shocks["sector"] == "OTHER"])["shock"]
                )
        ...
        gross_income = {
            "Household Group": households,
            "Y0": Y0.loc[households].sort_index(),
            "YL": YL.loc[households].sort_index(),
        }
        hh = {
            "Household Group": households[:5],
            "HH0": HH0.loc[households].sort_index(),
            "HHL": HHL.loc[households].sort_index(),
        }
        ds = {
            "Sectors": sectors,
            "DS0": DS0.loc[sectors_cap].sort_index(),
