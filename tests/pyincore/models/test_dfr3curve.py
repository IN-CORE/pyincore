...
def test_fragility_set_small_overlap():
    ...
    limit_states = collections.OrderedDict(
        [("LS_0", 0.9692754643), ("LS_1", 0.0001444974), ("LS_2", 0.0004277083)]
    )
    limit_states = AnalysisUtil.float_dict_to_decimal(limit_states)
    damage_states = fragility_set._3ls_to_4ds(limit_states)
    assert (
        damage_states["DS_0"] == AnalysisUtil.float_to_decimal(0.0307245357)
        and damage_states["DS_1"] == AnalysisUtil.float_to_decimal(0.968847756)
        and damage_states["DS_2"] == AnalysisUtil.float_to_decimal(0.0)
        and damage_states["DS_3"] == AnalysisUtil.float_to_decimal(0.0004277083)
    )

    # Test Case 2 - double overlap
    limit_states = collections.OrderedDict(
        [("LS_0", 0.12), ("LS_1", 0.64), ("LS_2", 0.8)]
    )
    limit_states = AnalysisUtil.float_dict_to_decimal(limit_states)
    damage_states = fragility_set._3ls_to_4ds(limit_states)
    assert (
        damage_states["DS_0"] == AnalysisUtil.float_to_decimal(0.2)
        and damage_states["DS_1"] == AnalysisUtil.float_to_decimal(0.0)
        and damage_states["DS_2"] == AnalysisUtil.float_to_decimal(0.0)
        and damage_states["DS_3"] == AnalysisUtil.float_to_decimal(0.8)
    )


def get_fragility_set(fragility_dir: str):
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, fragility_dir), "r", encoding="utf-8"
    ) as f:
...
def get_repair_set(repair_dir: str):
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, repair_dir), "r", encoding="utf-8"
    ) as f:
...
def get_restoration_set(restoration_dir: str):
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, restoration_dir), "r", encoding="utf-8"
    ) as f:
...
@pytest.mark.parametrize(
    "fragility_set,hazard_values,args,expected",
    [
        (get_fragility_set("fragility_curve.json"), {}, {}, 0.2619967240482869),
        (
            get_fragility_set("fragility_curve.json"),
            {"surgeLevel": 6, "waveHeight": 4},
            {},
            1.0,
        ),
        ...
        (
            get_fragility_set(
                "fragility_curves/PeriodStandardFragilityCurve_refactored.json"
            ),
            {"0.2 sec Sa": 4},
            {},
            0.9905435183,
...
def test_calculate_limit_state_probability(
    fragility_set, hazard_values, args, expected
):
...
@pytest.mark.parametrize(
    "repair_set,args,expected",
    [
        (get_repair_set("repairset.json"), {"repair_time": [15, 30]}, [0.227, 0.729]),
        (get_repair_set("repairset.json"), {"repair_time": 80}, 0.9943516689414926),
        (
            get_remote_repair_set("60edf9a4fc0f3a7af53a2194"),
            {"repair_time": [15, 30]},
            [0.989, 1],
        ),
    ],
...
@pytest.mark.parametrize(
    "restoration_set,args,expected",
    [
        (
            get_restoration_set("restorationset.json"),
            {"time": [15, 30]},
            [0.227, 0.729],
        ),
...
@pytest.mark.parametrize(
    ...
    [
        (
            get_restoration_set("pipe_restorationset.json"),
            {"break_rate": 0.2, "leak_rate": 0.4, "pipe_length": 80, "num_workers": 8},
            3.3000000000000003,
        )
    ],
)
