...
def test_create_flood_from_remote():
    ...
    values = flood.read_hazard_values(payload, hazard_service=hazardsvc, timeout=(30, 600))
...
def test_create_tsunami_from_remote():
    ...
    values = tsunami.read_hazard_values(payload, hazard_service=hazardsvc, timeout=(30, 600))
...
def test_create_eq_from_remote():
    ...
    response = eq.read_hazard_values(payload, hazard_service=hazardsvc, timeout=(30, 600))
...
def test_create_model_based_tornado_from_remote():
    ...
    response = tornado.read_hazard_values(payload, hazard_service=hazardsvc, timeout=(30, 600))
    assert len(response) == len(payload) \
           and len(response[0]['demands']) == len(payload[0]['demands']) \
           and response[0]['units'] == payload[0]['units'] \
           and len(response[0]['hazardValues']) == len(response[0]['demands']) \
