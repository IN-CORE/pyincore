...
def run_with_base_class():
    ...
    eq = Earthquake.from_hazard_service(
        "5b902cb273c3371e1236b36b", hazard_service=hazard_service
    )
    ...
    for bridge_name, highway_bridge_id in highway_bridges_id_list.items():
        ...
        for (
            component_name,
            highway_bridge_mapping_id,
        ) in highway_bridge_mapping_id_list.items():
            mapping_set = MappingSet(
                fragility_service.get_mapping(highway_bridge_mapping_id)
            )
    ...
    for bridge_name, railway_bridge_id in railway_bridges_id_list.items():
        ...
        for (
            component_name,
            railway_bridge_mapping_id,
        ) in railway_bridge_mapping_id_list.items():
            mapping_set = MappingSet(
                fragility_service.get_mapping(railway_bridge_mapping_id)
            )
            bridge_dmg.set_input_dataset("dfr3_mapping_set", mapping_set)
