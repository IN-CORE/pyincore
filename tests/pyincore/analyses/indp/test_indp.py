...
def run_with_base_class():
    ...
    epfs_id = "5d263f08b9219cf93c056c68"
    num_cpu = 8
    sim_number = 10
    ...
    fragility_service = FragilityService(prod_client)
    epf_dmg.load_remote_input_dataset("epfs", epfs_id)
    ...
    #
    # epf_df = Dataset.from_data_service("epfs_id", prod_datasvc).get_dataframe_from_shapefile()
    # epf_repair_time_df = pd.DataFrame()
    # for index, row in epf_df.iterrows:
    #     temp_dict = {'repairtime': epf_rest_util.get_restoration_time(guid=row["guid"], damage_state="DS_1", pf=0.99),
    #                  'guid': row["guid"]}
    #     epf_repair_time_df = epf_repair_time_df.append(temp_dict, ignore_index=True)
