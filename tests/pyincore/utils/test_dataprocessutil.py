...
def test_get_mapped_result(client):
    ...
    dmg_ret_json, func_ret_json, max_state_df = util.get_mapped_result_from_dataset_id(
        ...
        bldg_dataset_id,
        bldg_dmg_dataset_id,
        bldg_func_state_dataset_id,
        archetype_id,
    )

    assert "by_cluster" in dmg_ret_json.keys() and "by_category" in dmg_ret_json.keys()
    assert (
        "by_cluster" in func_ret_json.keys() and "by_category" in func_ret_json.keys()
    )
...
def test_get_mapped_result_from_analysis(client):
    ...
    dmg_result_dataset = Dataset.from_data_service(
        bldg_dmg_dataset_id, DataService(client)
    )

    archetype_id = "5fca915fb34b193f7a44059b"

    bldg_func_state_dataset_id = "660d9435ce705a7e547a664e"
    bldg_func_state_dataset = Dataset.from_data_service(
        bldg_func_state_dataset_id, DataService(client)
    )
...
def _functionality_cluster(
    client,
    archetype_mapping="5fca915fb34b193f7a44059b",
    ...
    archetype_mapping_dataset = Dataset.from_data_service(
        archetype_mapping, dataservice
    )
    ...
    ret_json = util.create_mapped_func_result(
        buildings, bldg_func_state, arch_mapping, arch_column
    )
...
def test_galveston_mcs_cluster(client):
    _functionality_cluster(
        client,
        archetype_mapping="6420befbb18d026e7c7dbafc",
