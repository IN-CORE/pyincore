...
def test_get_datasets(datasvc):
    ...
    metadata = datasvc.get_datasets(
        datatype=datatype, title="Shelby_County_Essential_Facilities"
    )

    if "id" not in metadata[0].keys():
        errors.append("response is not right!")
    if not re.search(
        r"shelby_county_essential_facilities", metadata[0]["title"].lower()
    ):
...
def test_create_dataset_shpfile(datasvc):
    ...
    print("dataset is created with id " + dataset_id)
    files = [
        str(os.path.join(pyglobals.TEST_DATA_DIR, "shp_sample/shp_sample.shp")),
...
def test_create_network_dataset(datasvc):
    ...
    files = [
        str(os.path.join(pyglobals.TEST_DATA_DIR, "shp_sample/network_links.shp")),
    ...
    response = datasvc.add_files_to_network_dataset(
        dataset_id, files, nodename, linkname, graphname
    )
...
def test_create_dataset_from_json_str(datasvc):
    with open(
        os.path.join(
            pyglobals.PYINCORE_ROOT_FOLDER, "tests/data/dataset_metadata.json"
        ),
        "r",
    ) as f:
    ...
    assert (
        Dataset.from_json_str(metadata_json_str, data_service=datasvc).local_file_path
        is not None
    )
    assert (
        Dataset.from_json_str(
            metadata_json_str, file_path="some_local_file_path"
        ).local_file_path
        is not None
    )
