...
def test_get_earthquake_hazard_value_set(hazardsvc):
    ...
    x, y, hazard_val = hazardsvc.get_earthquake_hazard_value_set(
        ...
        "0.2 SA",
        "g",
        [[-90.3099, 34.9942], [-89.6231, 35.4129]],
        0.01696,
    )
    assert (
...
def test_post_earthquake_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_earthquake_hazard_values(
        "5b902cb273c3371e1236b36b", payload
    )
    ...
    assert (
        ...
        and all(
            isinstance(hazardval, float)
            for hazardval in response[0]["hazardValues"]
        )
...
def test_bad_units_post_earthquake_hazard_values(hazardsvc):
    ...
    assert (
        len(response) == len(payload)
        and response[0]["hazardValues"][1] == -9999.3
    )
...
def test_get_liquefaction_values(hazardsvc):
    liq_vals = hazardsvc.get_liquefaction_values(
        "5b902cb273c3371e1236b36b",
    ...
    assert (
        liq_vals[0]["pgd"] == 94.28155130685825
        and liq_vals[1]["pgd"] == 103.2176731165868
    )
...
def test_post_liquefaction_values(hazardsvc):
    ...
    response = hazardsvc.post_liquefaction_values(
        "5b902cb273c3371e1236b36b", "5a284f53c7d30d13bc08249c", payload
    )
    ...
    assert (
        ...
        and all(
            isinstance(hazardval, float)
            for hazardval in response[0]["pgdValues"]
        )
...
def test_get_soil_amplification_value(hazardsvc):
    ...
    """
    soil_amplification_value = hazardsvc.get_soil_amplification_value(
        "NEHRP", "5a284f20c7d30d13bc081aa6", 32.3547, -89.3985, "pga", 0.2, "A"
    )
...
def test_create_and_delete_earthquake(hazardsvc):
    ...
    # print(pyglobals.TEST_DATA_DIR)
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "eq-dataset.json"), "r"
    ) as file:
        eq_dataset_json = file.read()

    file_paths = [
        str(os.path.join(pyglobals.TEST_DATA_DIR, "eq-dataset-SA.tif")),
    ...
    assert (
        dataset_response["id"] is not None
        and dataset_response["hazardDatasets"][1]["datasetId"] is not None
    )
    ...
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "eq-model.json"), "r"
    ) as file:
...
def test_get_earthquake_aleatory_uncertainty(hazardsvc):
    ...
    demand_type = "PGA"
    model_response = hazardsvc.get_earthquake_aleatory_uncertainty(
        hazard_id, demand_type
    )
...
def test_create_and_delete_tornado_scenario(hazardsvc):
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "tornado.json"), "r"
    ) as file:
        scenario = file.read()
...
def test_post_tornado_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_tornado_hazard_values(
        "5df913b83494fe000861a743", payload
    )
...
def test_post_tsunami_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_tsunami_hazard_values(
        "5bc9ead7f7b08533c7e610e0", payload
    )
...
def test_create_and_delete_tsunami_hazard(hazardsvc):
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "tsunami.json"), "r"
    ) as file:
        tsunami_json = file.read()

    file_paths = [
        str(os.path.join(pyglobals.TEST_DATA_DIR, "Tsu_100yr_Vmax.tif")),
    ...
    assert (
        ...
        and response["hazardDatasets"][1]["datasetId"] is not None
    )
...
def test_create_and_delete_hurricane(hazardsvc):
    ...
    """
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "hurricane-dataset.json"), "r"
    ) as file:
        hurricane_json = file.read()

    file_paths = [
        str(os.path.join(pyglobals.TEST_DATA_DIR, "Wave_Raster.tif")),
    ...
    assert (
        post_response["id"] is not None
        and post_response["hazardDatasets"][1]["datasetId"] is not None
    )
...
def test_post_hurricane_values(hazardsvc):
    ...
    response = hazardsvc.post_hurricane_hazard_values(
        "5f10837c01d3241d77729a4f", payload
    )
    ...
    assert (
        ...
        and response[0]["hazardValues"]
        == [1.54217780024576, 3.663398872786693]
    )
...
def test_create_and_delete_flood(hazardsvc):
    ...
    """
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "flood-dataset.json"), "r"
    ) as file:
    ...
    file_paths = [
        str(
            os.path.join(
                pyglobals.TEST_DATA_DIR, "flood-inundationDepth-50ft.tif"
            )
        ),
...
def test_post_flood_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_flood_hazard_values(
        "5f4d02e99f43ee0dde768406", payload
    )
...
def test_create_and_delete_hurricane_windfield(hazardsvc):
    with open(
        os.path.join(pyglobals.TEST_DATA_DIR, "hurricanewf.json"), "r"
    ) as file:
...
def test_post_hurricanewf_hazard_values(hazardsvc):
    payload = [
        {"demands": ["3s", "60s"], "units": ["mph", "mph"], "loc": "28,-81"}
    ]
    elevation = 10
    roughness = 0.03

    response = hazardsvc.post_hurricanewf_hazard_values(
        "5cffdcf35648c404a6414f7e", payload, elevation, roughness
    )
    ...
    assert (
        ...
        and response[0]["hazardValues"]
        == [81.57440785011988, 66.83292578974765]
    )


@pytest.mark.skip(reason="performance issues")
def test_get_hurricanewf_json(hazardsvc):
    hjson = hazardsvc.get_hurricanewf_json(
        "florida", 1, -83, "28,-81", "3s", "kmph", 6, 10, "circular"
    )
