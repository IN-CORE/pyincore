...
def test_bad_units_post_earthquake_hazard_values(hazardsvc):
    payload = [
        {"demands": ["1.0 SD", "0.2 SA"], "units": ["cm", "zzz"], "loc": "35.84,-89.90"}
    ]
    response = hazardsvc.post_earthquake_hazard_values(
        "5b902cb273c3371e1236b36b", payload
    )
...
def test_bad_format_post_earthquake_hazard_values(hazardsvc):
    ...
    try:
        hazardsvc.post_earthquake_hazard_values("5b902cb273c3371e1236b36b", payload)
...
def test_post_liquefaction_values(hazardsvc):
    payload = [
        {"demands": ["pgd", "pgd"], "units": ["cm", "in"], "loc": "35.19,-90.0178"}
    ]
...
def test_get_earthquake_variance(hazardsvc):
    ...
    assert model_response[0] is not None and (0 < model_response[0]["variance"] <= 1)
...
def test_get_tornado_hazard_metadata(hazardsvc):
    ...
    """
    response = hazardsvc.get_tornado_hazard_metadata("5df913b83494fe000861a743")
...
def test_post_tornado_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_tornado_hazard_values("5df913b83494fe000861a743", payload)
...
def test_post_tsunami_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_tsunami_hazard_values("5bc9ead7f7b08533c7e610e0", payload)
...
def test_create_and_delete_flood(hazardsvc):
    ...
    file_paths = [
        str(os.path.join(pyglobals.TEST_DATA_DIR, "flood-inundationDepth-50ft.tif")),
...
def test_post_flood_hazard_values(hazardsvc):
    ...
    response = hazardsvc.post_flood_hazard_values("5f4d02e99f43ee0dde768406", payload)
...
def test_post_hurricanewf_hazard_values(hazardsvc):
    payload = [{"demands": ["3s", "60s"], "units": ["mph", "mph"], "loc": "28,-81"}]
    elevation = 10
    roughness = 0.03
